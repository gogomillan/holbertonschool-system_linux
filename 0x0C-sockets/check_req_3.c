...
{
	char *c_methods[] = {"POST", "GET", "DELETE", NULL};
	char *method = NULL, *path = NULL, *c_paths[] = {"/todos", NULL};
	int m = 0, p = 0;
	...
	if (method && path)
	{
		switch (m)
		{
			...
			case GET:
				...
				break;
			case DELETE:
				req_data->status = delete_method(req_data);
				break;
		}
...
void create_res(int status, http_msg_t *req_data, char *res)
{
	...
	if (method)
		return;
	method = delete_res(status, req_data, res);
	if (method)
		return;
}
